export const formatCpfOrCnpj = (value: string, type?: string): string => {
  ...
  const formatCpf = (cpf: string) => {
    ...
    if (length <= 9) return `${cpf.slice(0, 3)}.${cpf.slice(3, 6)}.${cpf.slice(6)}`;
  ...
  const formatCnpj = (cnpj: string) => {
    ...
    if (length <= 8) return `${cnpj.slice(0, 2)}.${cnpj.slice(2, 5)}.${cnpj.slice(5)}`;
    if (length <= 12) return `${cnpj.slice(0, 2)}.${cnpj.slice(2, 5)}.${cnpj.slice(5, 8)}/${cnpj.slice(8)}`;
